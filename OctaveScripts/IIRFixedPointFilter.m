...
function output = IIRSLAFixedPointFilter (numQuant, denQuant, iirQuant, input)
  ...
  for ii = 1 : length(fir_response)
    temp = fir_response_shifted(ii)*denQuant(1) - denQuant(3) * iir_states(2) - denQuant(5) * iir_states(4) - fraction_saving;
    temp2 = floor(temp/(2^(iirQuant.nbFracBitsCoef)));
    fraction_saving = temp2*(2^(iirQuant.nbFracBitsCoef)) - temp;
    all_pole_response(ii) = round(temp2/(2^iirQuant.nbAddPrecisionBits));
    for jj = 3 :-1:1
  ...
  output =  round(all_pole_response(1:length(input))/(2^(iirQuant.nbFracBitsCoef)));
endfunction
